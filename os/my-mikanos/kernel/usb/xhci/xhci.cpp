...
Error AddressDevice(Controller& xhc, uint8_t port_id, uint8_t slot_id)
{
    ...
    memset(
        &dev->InputContext()->input_control_context, 0,
...
void SwitchEhci2Xhci(const pci::Device& xhc_dev)
{
    ...
    Log(
        kDebug, "SwitchEhci2Xhci: SS = %02x, xHCI = %02x\n",
...
Controller::Controller(uintptr_t mmio_base)
    : mmio_base_{mmio_base},
      ...
      op_{
          reinterpret_cast<OperationalRegisters*>(
...
Error ConfigureEndpoints(Controller& xhc, Device& dev)
{
    ...
    auto convert_interval{
        (port_speed == kFullSpeed || port_speed == kLowSpeed)
            ? [](EndpointType type, int interval)
            {
                // for FS, LS
...
void Initialize()
{
    ...
    for (int i = 1; i <= xhc.MaxPorts(); ++i)
    {
        ...
        if (port.IsConnected())
        {
            ...
            {
                Log(
                    kError, "failed to configure port: %s at %s:%d\n",
...
void ProcessEvents()
{
    while (controller->PrimaryEventRing()->HasFront())
    {
        ...
        {
            Log(
                kError, "Error while ProcessEvent: %s at %s:%d\n",
                err.Name(), err.File(), err.Line()
            );
        }
    }
