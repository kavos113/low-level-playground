...
{
    Log(
        level, "Interface Descriptor %d: num_ep=%d class=%d sub=%d protocol=%d\n",
...
{
    Log(
        level, "EndpointConf: ep_id=%d, ep_type=%d"
...
{
    Log(
        level, "HID Descriptor: release=0x%02x, num_desc=%d",
...
Error Device::StartInitialize()
{
    ...
    return GetDescriptor(
        *this, kDefaultControlPipeID, DeviceDescriptor::kType, 0,
...
{
    Log(
        kDebug, "Device::OnControlCompleted: buf 0x%08lx, len %d, dir %d\n",
